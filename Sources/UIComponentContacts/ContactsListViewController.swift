...
public class ContactsListViewController: UITableViewController {
    ...
    private lazy var listDataSource = ContactsDataSource(
        query: query ?? ContactsQuery(type: .followers))
    ...
    private lazy var dataSource = UITableViewDiffableDataSource<SingleListSection, ContactUser>(
        tableView: tableView
...
extension ContactsListViewController {
    ...
    private func configureHierarchy() {
        ...
        refreshControl.addTarget(
            self, action: #selector(refreshContacts),
            for: .valueChanged)
