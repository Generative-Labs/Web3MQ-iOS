...
public class NotificationListViewController: UIViewController {
    ...
    private lazy var notificationDataSource = NotificationDataStore(
        query: query ?? NotificationQuery(types: [], isAscending: true))
    ...
    private lazy var dataSource = UITableViewDiffableDataSource<SingleListSection, Notification>(
        tableView: tableView
...
extension NotificationListViewController: NotificationTableViewCellDelegate {
    ...
    func notificationCell(
        _ cell: NotificationBaseTableViewCell,
