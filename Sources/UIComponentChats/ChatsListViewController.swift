...
public protocol ChatsListViewControllerDelegate: AnyObject {
    ...
    func chatsListViewController(_ viewController: ChatsListViewController,
...
open class ChatsListViewController: UIViewController {
    ...
    private lazy var chatsListDataSource = ChatsListDataSource(query: query ?? ChatsListQuery(types: ChannelType.all))
    
    ///
    private lazy var emptyView = EmptyView(image: UIImage(systemName: "ellipsis.bubble"),
                                           title: "Your message list is empty")
    
    ///
    private let tableView = UITableView()
    
    private lazy var dataSource = EditableTableViewDataSource(tableView: tableView) { tableView, indexPath, itemIdentifier in
